...
def map_type(python_type: type) -> type:
    ...
    if issubclass(origin_type, list):
        ...
        # TODO: Nested list support
        if not aq_types.is_qasm_type(item_type) or isinstance(item_type, oqpy.ArrayVar):
            raise errors.ParameterTypeError(
                f"Unsupported array type: {item_type}. "
                "AutoQASM Arrays support bool, int, and float."
            )
        ...
        return oqpy.ArrayVar[item_type, 10]
